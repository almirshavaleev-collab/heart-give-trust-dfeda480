...
import { structuredLog } from "../_shared/recurring.ts";
...
Deno.serve(async (req) => {
  ...
  const h24 = new Date(now - 24 * 3600e3).toISOString();
  ...
  // 5. Stale processing_at locks
  {
    const { count, error } = await supabase
      ...
      .update({ processing_at: null }, { count: "exact" })
      .lt("processing_at", m10)
      .not("processing_at", "is", null);
    if (error) structuredLog("cleanup_error", { step: "stale_processing", msg: error.message });
    result.stale_processing_cleared = count;
  }
